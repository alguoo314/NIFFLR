...
e transcripts")
    ...
    global ref_exon_chain_length_record
    ref_exon_chain_length_record={}
...
def process_ref_transcript(ref_line,ref_line_fields,coord_starts,first_exon_end,chr_name,transcript_len):
    ...
    while ref_line != "" and int(ref_line_fields[3]) <  first_exon_end and ref_line_fields[0] == chr_name:
        ...
        ref_exon_chain_length_record[transcript_id]=transcript_len
...
def calc_read_proportions(assembled_exon_chain,num_reads,single_exon,max_read_len,end_pos,num_junc): #end_pos is the position of the 3' end of the transcript
    ...
    if sum(coverages) == len(coverages):
        inverse_lens = [1 / x for x in ref_lens]
        scaling_factor = num_reads / sum(inverse_lens)
        coverage_proportions = [scaling_factor * (1 / x) for x in ref_lens]
